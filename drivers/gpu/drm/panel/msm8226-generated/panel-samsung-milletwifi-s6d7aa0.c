...
static int sdc_probe(struct mipi_dsi_device *dsi)
{
	...
	ctx->supply = devm_regulator_get(dev, "vdd_supply");
	if (IS_ERR(ctx->supply))
		return dev_err_probe(dev, PTR_ERR(ctx->supply),
				     "Failed to get panel_supply regulator\n");
...
static const struct of_device_id sdc_of_match[] = {
	{ .compatible = "samsung,milletwifi-panel-s6d7aa0" },
	{ /* sentinel */ }
};
...
MODULE_AUTHOR("Bryant Mairs <bryant@mai.rs>");
